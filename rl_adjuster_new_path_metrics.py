...
class OnlineLearningPathEnv:
    ...
    def compute_final_quality(self, policy_weight: Optional[Dict[str,float]] = None) -> Dict[str, torch.Tensor]:
        """
        终止(terminal)评估：严格在 **RL 实际推荐路径(动作序列)** 上计算 Eq.(19)(20)(21)

        说明：
        - 之前版本使用 policy 分布的 topk_recs_policy 来构造 topk_tensor 并计算指标。
          这会导致“评估对象 != 实际输出路径”。
        - 本版本改为：仅使用 self.chosen_items (每步实际 chosen item) 构造 path_tensor，并在其上计算
          effectiveness / adaptivity / diversity / final_quality。
