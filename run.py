...
def test_epoch(model, validation_data, graph, hypergraph_list, kt_loss,kt_evaluator=None, k_list=[1, 5, 10, 20]):
    ...
    with torch.no_grad():
        for i, batch in enumerate(validation_data):  # tqdm(validation_data, mininterval=2, desc='  - (Validation) ', leave=False):
            ...
            # =========================================================
            # ✅ 新增：滑动窗口 EP 收益评估 (多时间点采样，更准确的平均 EP)
            # =========================================================
            valid_len = (tgt[0] > 1).sum().item()
            min_hist_len = 10   # 最小历史序列长度
            target_len = 5      # 目标窗口长度
            step = 3            # 滑动步长 (隔几个时间步评估一次)

            if valid_len >= min_hist_len + target_len:
                hidden_kt_eval = model.gnn(graph)  # 只需计算一次

                # 滑动窗口遍历：从第 min_hist_len 题开始，每隔 step 步评估一次
                for start_pos in range(min_hist_len, valid_len - target_len + 1, step):
                    # 1. 切分历史序列 [0, start_pos)
                    hist_seq = tgt[0:1, :start_pos].cuda()
                    hist_ans = ans[0:1, :start_pos].cuda()

                    # 2. 提取【真实学习目标 target_actual】 [start_pos, start_pos+target_len)
                    target_actual_seq = tgt[0:1, start_pos:start_pos + target_len]
                    ...
                    hist_pred, _, _, _, _, _ = model(
                        hist_seq, tgt_timestamp[0:1, :start_pos], tgt_idx[0:1, :start_pos],
                    ...
                    # 4. 计算初始掌握度 (当前历史序列结束时的状态)
                    _, _, yt_init_eval, _, _ = kt_loss.ktmodel(hidden_kt_eval, hist_seq, hist_ans) if hasattr(kt_loss, 'ktmodel') else model.ktmodel(hidden_kt_eval, hist_seq, hist_ans)
                    p_init = yt_init_eval[0, -1, :]

                    # 过滤学霸：对目标题平均掌握度 < 0.9 才评估
                    ep_init_abs = sum([p_init[t_id].item() for t_id in target_actual])
                    if ep_init_abs >= len(target_actual) * 0.9:
                        continue

                    # --- 5. 评估学生真实路径 (Base EP) ---
                    real_seq = tgt[0:1, :start_pos + target_len].cuda()
                    real_ans = ans[0:1, :start_pos + target_len].cuda()
                    ...
                    gen_path = generate_dynamic_ep_path(
                        ...
                        graph=graph,
                        max_length=target_len + 3,  # 允许稍长
                        candidate_size=100
                    )

                    # --- 7. 评估生成路径 (Opt EP) ---
                    if len(gen_path) > 0:
                        opt_seq = torch.cat([hist_seq, torch.tensor([gen_path], device='cuda')], dim=1)
                        opt_ans = torch.cat([hist_ans, torch.ones((1, len(gen_path)), device='cuda')], dim=1)
                        ...
                        print(f"\n[滑动窗口 @ pos={start_pos}] 预测目标: {target_pred[:3]}... | 真实目标: {target_actual}")
                        print(f"   => 真实路径: {target_actual} | EP: {ep_base:.4f}")
                        print(f"   => 算法路径 (len={len(gen_path)}): {gen_path[:5]}... | EP: {ep_opt:.4f}")
                        print(f"   => Delta: {delta_ep:+.4f}  {'🚀' if delta_ep > 0 else '📉'}")
            # =========================================================
            # # =========================================================
